...
export default function InvestmentsPage() {
  ...
  const totalCurrent = investments.reduce((acc, i) => acc + (i.currentValue || i.amount), 0);
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        <header className="flex-none bg-[#F8FAFC]/80 backdrop-blur-md px-4 md:px-8 py-4 flex justify-between items-center border-b border-gray-100/50 z-10">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl lg:text-2xl font-black tracking-tight text-black">Investimentos</h1>
          </div>
          <button 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
               {isLoading ? (
                 ...
                 investments.map((inv) => {
                   ...
                   const yieldPercent = invested > 0 ? ((current - invested) / invested) * 100 : 0;
                   const isPositive = profit >= 0;

                   return (
                   <div key={inv.id} className="group bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 flex flex-col justify-between min-h-[280px]">
                      <div className="flex justify-between items-start">
                         ...
                         <div className="flex items-center gap-2">
                            <div className={`text-[10px] font-black px-2 py-1 rounded-lg ${isPositive ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'}`}>
                            ...
                            {!inv.isVirtual && (
                              <div className="flex gap-1 md:opacity-0 group-hover:opacity-100 transition-all">
                                  ...
                                  <button onClick={() => handleDelete(inv.id)} className="p-2 hover:bg-rose-50 rounded-xl text-gray-400 hover:text-rose-600 transition-colors">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                  </button>
                              </div>
                            )}
                            {inv.isVirtual && (
                               <div className="bg-gray-100 text-gray-400 p-2 rounded-xl" title="Gerenciado na conta">
                      ...
                      <div className="mt-6">
                        ...
                        <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mt-1">{inv.accountName}</p>
                        
                        <div className="mt-6 space-y-4">
                           <div className="flex justify-between items-end border-b border-gray-50 pb-4">
                              <div>
                                <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest leading-none mb-1">Investido</p>
                                <p className="text-sm font-bold text-gray-500 tabular-nums">{formatCurrency(invested)}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest leading-none mb-1">Resultado</p>
                                <p className={`text-sm font-black tabular-nums ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
                           ...
                           <div className="flex flex-col">
                              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Valor Atual</span>
                              <span className="text-2xl md:text-3xl font-black tabular-nums text-gray-900 tracking-tighter">{formatCurrency(current)}</span>
                           </div>
                        </div>
                      </div>
                   </div>
