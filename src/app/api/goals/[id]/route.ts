...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.$executeRaw`
      UPDATE "Goal" 
      SET name = ${body.name}, "targetAmount" = ${Number(body.targetAmount)}, "currentAmount" = ${Number(body.currentAmount)}, deadline = ${deadline}::timestamp, color = ${body.color}, "updatedAt" = ${now}::timestamp
      WHERE id = ${id}
    `;
