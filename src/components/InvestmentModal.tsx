  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ name, type, amount: parseFloat(amount), accountId }),
      });
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 md:p-4">
      ...
      <div className="relative bg-white w-full max-w-md max-h-[95dvh] md:max-h-[90vh] overflow-hidden rounded-[2rem] md:rounded-[2.5rem] shadow-2xl transform animate-in zoom-in-95 duration-300 flex flex-col">
        ...
        <div className="flex-none p-6 md:p-8 pb-4 flex justify-between items-center border-b border-gray-50">
          <div>
            <h2 className="text-xl md:text-2xl font-black text-gray-900 tracking-tight">{investment ? 'Editar Ativo' : 'Novo Investimento'}</h2>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Gerencie seu patrimônio</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 pt-6 custom-scrollbar">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Valor (R$)</label>
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Vincular à Conta</label>
            ...
            <div className="pt-4 flex flex-col gap-3">
              ...
              >
                {isLoading ? 'Processando...' : 'Salvar Ativo'}
              </button>
              <button 
                type="button" 
                onClick={onClose} 
