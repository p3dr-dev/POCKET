...
- **Infraestrutura Remota:** O banco de dados é um PostgreSQL gerenciado pelo **Supabase**. Nunca assuma que o banco é SQLite ou local.
- **Prisma Client & Raw SQL:** O uso de **Prisma Client (`prisma.user.create`, etc.) é mandatório para operações de ESCRITA**. O `prisma.$executeRaw` deve ser usado apenas quando houver necessidade de performance extrema ou recursos específicos do Postgres não mapeados.
- **Ambiente Serverless (Vercel):** O projeto é otimizado para execução *serverless*. NUNCA dependa do sistema de arquivos local (`fs`) para persistência de dados. Utilize variáveis de ambiente para todas as URLs de conexão.

### 3.2. Fluxo de Desenvolvimento
- **GitHub:** Todo o código deve seguir o fluxo de branches e Pull Requests. Nunca envie segredos (.env) para o repositório.
- **Vercel Deploy:** As rotas de API devem ser rápidas para evitar timeouts das Edge/Serverless Functions.
